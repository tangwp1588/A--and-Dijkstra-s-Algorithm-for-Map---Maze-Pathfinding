...
struct cell {
    ...
    double dist;  // Changed from f,g,h to just dist for Dijkstra
};
...
vector<Pair> dijkstraSearch(Pair src, Pair dest) {
        ...
        for (int dir = 0; dir < 8; dir++) {
            ...
            if (isValid(newRow, newCol)) {
                if (isDestination(newRow, newCol, dest)) {
                    ...
                    // Calculate new distance (1.0 for orthogonal, 1.414 for diagonal)
                    double newDist = cellDetails[i][j].dist + ((dir < 4) ? 1.0 : 1.414);

                    if (newDist < cellDetails[newRow][newCol].dist) {
                        cellDetails[newRow][newCol].dist = newDist;
                        cellDetails[newRow][newCol].parent_i = i;
                        cellDetails[newRow][newCol].parent_j = j;
...
vector<Pair> handleNewObstacle(int row, int col, Pair current, Pair dest) {
    ...
    vector<Pair> newPath = dijkstraSearch(current, dest);
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                ...
                if (isValid(newRow, newCol) && isUnBlocked(newRow, newCol)) {
                    ...
                    newPath = dijkstraSearch(altStart, dest);
                    if (!newPath.empty()) {
...
int main() {
    ...
    try {
        ...
        vector<Pair> initialPath = dijkstraSearch(src, dest);
